...
### 佈署（Deploy）到預發佈（Staging）環境以及正式（Production）環境的方法

當 `master` 分支被更新時，HTML 檔案將會在 `production` 分支中被組建（build）起來，接著正式（Production）網站 https://stopcovid19.metro.tokyo.lg.jp/ 會被更新。

當 `staging` 分支被更新時，HTML 檔案將會在 `gh-pages` 分支中被組建（build）起來，接著預發佈（Staging）網站 https://stg-covid19-tokyo.netlify.com/ 會被更新。

當 `development` 分支被更新時，HTML 檔案將會在 `dev-pages` 分支中被組建（build）起來，接著開發（Development）網站 https://dev-covid19-tokyo.netlify.com/ 會被更新。
